...
public:
    ...
    void RenderMenu() {
        ...
        // 2. Setup 2D projection for all text rendering
        ...
        // --- Instructions Block (New) ---
        float instructionScale = 0.75f;
        glm::vec3 instructionColor = glm::vec3(0.7f);
        float startY = centerY + 50.0f; // Start instructions below the subtitle
        float lineSpacing = 40.0f;

        std::string line1 = "W / S - Move in the 4th Dimension";
        std::string line2 = "Left Click - Place Fruit";
        std::string line3 = "Right-Click + Drag - Rotate Camera";
        std::string line4 = "Mouse Wheel - Zoom In/Out";
        std::string line5 = "ESC - Pause / Return to Menu";

        t_rend->RenderText(line1, centerX - t_rend->GetTextWidth(line1, instructionScale) / 2.0f, startY, instructionScale, instructionColor);
        t_rend->RenderText(line2, centerX - t_rend->GetTextWidth(line2, instructionScale) / 2.0f, startY + lineSpacing, instructionScale, instructionColor);
        t_rend->RenderText(line3, centerX - t_rend->GetTextWidth(line3, instructionScale) / 2.0f, startY + (lineSpacing * 2), instructionScale, instructionColor);
        t_rend->RenderText(line4, centerX - t_rend->GetTextWidth(line4, instructionScale) / 2.0f, startY + (lineSpacing * 3), instructionScale, instructionColor);
        t_rend->RenderText(line5, centerX - t_rend->GetTextWidth(line5, instructionScale) / 2.0f, startY + (lineSpacing * 4), instructionScale, instructionColor);
    }
